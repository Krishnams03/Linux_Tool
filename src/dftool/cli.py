"""
DFTool - CLI Query & Management Tool
Command-line interface for querying forensic logs, viewing alerts,
generating timeline reports, and managing the daemon.

Usage:
    dftool status                                # daemon status
    dftool alerts [--severity HIGH] [--last 1h]  # view alerts
    dftool timeline [--from ... --to ...]        # event timeline
    dftool search --query "ssh root"             # search logs
    dftool export --format csv --output out.csv  # export evidence
    dftool verify                                # verify log integrity
"""
...
def read_log_entries(log_path: str, max_entries: int = 10000) -> list[dict]:
    """Read JSON log entries from a dftool log file."""
    entries = []
...
def cmd_status(args, config):
    ...
    if pid:
        _print_success(f"DFTool daemon is running (pid {pid})")
    else:
        _print_warn("DFTool daemon is NOT running")
    # Show log sizes
...
def cmd_alerts(args, config):
    ...
    # Display
    if HAS_RICH:
        table = Table(title="DFTool Alerts", box=box.ROUNDED, show_lines=True)
...
def cmd_export(args, config):
    ...
    log_dir = config["logging"]["log_dir"]
    main_log = os.path.join(log_dir, "dftool.log")
    alert_log = config.get("alerts", {}).get("alert_log", "/var/log/dftool/alerts.log")
...
def main():
    parser = argparse.ArgumentParser(
        prog="dftool",
        description="DFTool - Digital Forensics Log Query & Management CLI",
    )
    parser.add_argument(
        "--config", "-c",
