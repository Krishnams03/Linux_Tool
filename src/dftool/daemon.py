"""
DFTool - Daemon Entry Point
Main daemon process that initializes all monitors and runs them
as a background service.

Usage:
    dftoold start [--config /path/to/config.yaml] [--foreground]
    dftoold stop
    dftoold status
"""
...
logger = logging.getLogger("dftool.daemon")
...
class DFToolDaemon:
    ...
    def start(self, foreground: bool = False):
        ...
        self._alert_engine.fire(
            monitor="daemon",
            event_type="DAEMON_START",
            message="DFTool forensic monitoring daemon started",
        ...
        logger.info("DFTool daemon running (pid %d) — %d monitors active",
    ...
    def stop(self):
        ...
        logger.info("DFTool daemon shutting down …")
        self._running = False
        ...
        if self._alert_engine:
            self._alert_engine.fire(
                ...
                message="DFTool daemon stopped gracefully",
                severity=SEVERITY_INFO,
            )

        self._remove_pid()
        logger.info("DFTool daemon stopped")
...
def main():
    parser = argparse.ArgumentParser(
        description="DFTool - Digital Forensics Monitoring Daemon"
    )
    parser.add_argument(
        "action",
    ...
    if args.action == "start":
        ...
        if running:
            print(f"DFTool daemon already running (pid {running})")
            sys.exit(1)
        print("Starting DFTool daemon …")
        daemon = DFToolDaemon(config)
        daemon.start(foreground=args.foreground)

    elif args.action == "stop":
        pid = get_running_pid(pid_file)
        if pid is None:
            print("DFTool daemon is not running")
            sys.exit(1)
        print(f"Stopping DFTool daemon (pid {pid}) …")
        ...
        pid = get_running_pid(pid_file)
        if pid:
            print(f"DFTool daemon is running (pid {pid})")
        else:
            print("DFTool daemon is not running")
            sys.exit(1)
